...
function defaultAiSettings(organizationId) {
...
export async function getOrganizationAiSettings(organizationId) {
...
export async function updateOrganizationAiSettings(organizationId, patch) {
    const { data, error } = await supabaseAdmin
        .from('organization_ai_settings')
        .upsert({
        organization_id: organizationId,
...
export async function getAdminAiStatusSummary() {
    const [automationEnabledCountResult, ticketClassificationEnabledCountResult, reminderGenerationEnabledCountResult, ticketSummarizationEnabledCountResult,] = await Promise.all([
